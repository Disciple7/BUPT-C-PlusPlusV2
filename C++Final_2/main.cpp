...
int main(int argc, char** argv)
{
	...
	while (choice)
	{
		...
		case 1://注册模式
		{
			state_register(playerList,testerList);
			break;
		}
		case 2://游戏模式。登录后获得
		{
			nowUserPos = player_login(playerList);
			if (nowUserPos!=-1)
			{
				bestRound = state_game(wordList);
				flush_player(nowUserPos, bestRound, playerList);//获得用户成绩，刷新用户数据
			}
			break;
		}
		case 3:
		{
			nowUserPos = tester_login(testerList);
			if (nowUserPos!=-1)
			{
				wordAdd = state_testmake(wordList);
				flush_tester(nowUserPos, wordAdd, testerList);//获得用户贡献，刷新用户数据
			}
			break;
		}
		case 4:
		{
			state_query(playerList,testerList);//state_query会改变存储的用户位置。每一次查询后都要重置nowUserPos
			break;
		}
		case 0:
		{
